...
def _store_ingestion(result: dict) -> None:
    ...
    st.session_state.ingestion_result = {
        ...
        "stereo_seq_readiness": result.get("stereo_seq_readiness"),
        "stereo_seq": result.get("stereo_seq"),
    }
    st.session_state.mbsi_platform = platform
    st.session_state.mbsi_readiness = readiness

    if platform == "stereo_seq":
        st.session_state.stereo_seq_readiness = result.get("stereo_seq_readiness", readiness)
        st.session_state.platform_metadata = {
            **(st.session_state.get("platform_metadata") or {}),
            "stereo_seq": result.get("stereo_seq", {}),
            "detection": detection,
        }
