"""Study Setup & Data Ingestion — technology-first ingestion workspace."""
...
def _render_technology_selection() -> str:
    st.markdown("#### 1. What spatial technology are you using?")
    ...
    if spec:
        with st.expander(f"{spec.label} — required files & hints", expanded=True):
            ...
            if spec.key == "stereo_seq":
                st.info(
                    "Stereo-seq: SAW/StereoMap, GEF/CGEF, registered H&E/mIF, segmentation, "
                    "lasso regions, and HTML QC reports are detected (partial parse support)."
                )
...
def render() -> None:
    _init_project_state()
    st.markdown("### Study Setup & Data Ingestion")
    st.caption("Select technology, describe the study, define samples, upload files, review readiness.")
    ...
    st.session_state.run_outputs["study_setup"] = run.to_dict()

    st.divider()
    _render_summary_card(recommended, compatibility_df, _uploaded_file_summary())

    if st.button("Continue to Data QC & Preprocessing", type="primary", key="ss_continue_qc"):
        st.session_state.active_module = WorkflowModule.QC_PREPROCESS.value
        st.rerun()
