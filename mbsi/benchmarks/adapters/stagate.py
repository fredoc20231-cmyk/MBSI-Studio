"""STAGATE adapter — baseline proxy."""
...
class STAGATEAdapter(BaseBenchmarkAdapter):
    name = "stagate"
    method_type = "proxy"
    ...
    def run(self, ground_truth_adata, pseudo_visium_adata) -> AdapterResult:
        recon = self._proxy_knn_from_spots(
            ...
            pseudo_visium_adata,
            k=3,
            label="stagate_baseline_proxy",
        )
        return AdapterResult(
            reconstructed_adata=recon,
            method=self.name,
            method_type="proxy",
            notes="STAGATE not installed; spatial interpolation baseline_proxy.",
        )
